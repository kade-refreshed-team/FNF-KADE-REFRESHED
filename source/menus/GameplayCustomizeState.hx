...
class GameplayCustomizeState extends base.MusicBeatState
{
    ...
    public override function create() {
        ...
        sick = new FlxSprite().loadGraphic(Paths.image('game-side/hitPopups/sick'));
        sick.scrollFactor.set();
        background = new FlxSprite(-600, -200).loadGraphic(Paths.image('stages/stage/stageback'));
        curt = new FlxSprite(-500, -300).loadGraphic(Paths.image('stages/stage/stagecurtains'));
        front = new FlxSprite(-650, 600).loadGraphic(Paths.image('stages/stage/stagefront'));
	...
	private function generateStaticArrows(player:Int):Void
        {
            for (i in 0...4)
            {
                ...
                babyArrow.frames = Paths.getSparrowAtlas('game-side/NOTE_assets', 'shared');
