...
{
	#if sys
	public static var foldersToCheck = ["./assets"];
	#end
	...
	**/
	...
	{
		#if sys
		var sysPath = (id.startsWith("assets/")) ? id.substr(7) : id;
	...
	**/
	public static function getBitmapData(id:String, useCache:Bool = true):BitmapData
	{
		#if sys
		var sysPath = (id.startsWith("assets/")) ? id.substr(7) : id;
		if (cache.sysBitmapCache.exists(sysPath))
			return cache.sysBitmapCache[sysPath];

		for (folder in foldersToCheck) {
			if (FileSystem.exists('$folder/$sysPath')) {
				var bitmap = BitmapData.fromFile('$folder/$sysPath');
				cache.sysBitmapCache.set(sysPath, bitmap);
				return bitmap;
			}
		}
		#end
	...
	**/
	public static function getBytes(id:String):ByteArray
	{
		#if sys
		var sysPath = (id.startsWith("assets/")) ? id.substr(7) : id;
		if (cache.sysByteCache.exists(sysPath))
			return cache.sysByteCache[sysPath];

		for (folder in foldersToCheck) {
			if (FileSystem.exists('$folder/$sysPath')) {
				var byteArray = ByteArray.fromFile('$folder/$sysPath');
				cache.sysByteCache.set(sysPath, byteArray);
				return byteArray;
			}
		}
		#end
	...
	**/
	public static function getFont(id:String, useCache:Bool = true):Font
	{
		#if sys
		var sysPath = (id.startsWith("assets/")) ? id.substr(7) : id;
		if (cache.sysFontCache.exists(sysPath))
			return cache.sysFontCache[sysPath];

		for (folder in foldersToCheck) {
			if (FileSystem.exists('$folder/$sysPath')) {
				var font = Font.fromFile('$folder/$sysPath');
				cache.sysFontCache.set(sysPath, font);
				return font;
			}
		}
		#end
	...
	**/
	public static function getSound(id:String, useCache:Bool = true):Sound
	{
		#if sys
		var sysPath = (id.startsWith("assets/")) ? id.substr(7) : id;
		if (cache.sysSoundCache.exists(sysPath))
			return cache.sysSoundCache[sysPath];

		for (folder in foldersToCheck) {
			if (FileSystem.exists('$folder/$sysPath')) {
				var sound = Sound.fromFile('$folder/$sysPath');
				cache.sysSoundCache.set(sysPath, sound);
				return sound;
			}
		}
		#end
