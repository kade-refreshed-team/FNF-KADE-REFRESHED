...
{
	#if sys
	public var sysBitmapCache:Map<String, BitmapData> = [];
	public var sysByteCache:Map<String, ByteArray> = [];
	public var sysFontCache:Map<String, Font> = [];
	public var sysSoundCache:Map<String, Sound> = []; //originally i accidentially had this named "Sopundp."
	//Just thought I mention bc its kinda funny
	#end
	...
	**/
	...
	{
		#if sys
		for (key in sysBitmapCache.keys()) {
			if (prefix == null || StringTools.startsWith(key, prefix))
				sysBitmapCache.remove(key);
		}
		for (key in sysByteCache.keys()) {
			if (prefix == null || StringTools.startsWith(key, prefix))
				sysByteCache.remove(key);
		}
		for (key in sysFontCache.keys()) {
			if (prefix == null || StringTools.startsWith(key, prefix))
				sysFontCache.remove(key);
		}
		for (key in sysSoundCache.keys()) {
			if (prefix == null || StringTools.startsWith(key, prefix))
				sysSoundCache.remove(key);
		}
		#end
