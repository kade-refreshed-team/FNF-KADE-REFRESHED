	...
	override public function updateFollow():Void
	{
		...
		else
		{
			...
			else
			{
				...
				scroll.x += (_scrollTarget.x - scroll.x) * CoolUtil.adjustToFramerate(followLerp);
				scroll.y += (_scrollTarget.y - scroll.y) * CoolUtil.adjustToFramerate(followLerp);
			}
		}
	}
}